...
function little_big_diner_setup() {
	...
	add_image_size('lbd-portrait', 535, 420, array('left', 'top'));
	add_image_size('lbd-landscape', 535, 210, array('center', 'center'));
...
function lbd_gallery_gen($g, $t) {
	if (function_exists('get_field')) {
		...
		$images = array();
		shuffle($g); // will keep image generation random
		for ($i = 0; $i < $t; $i++) {
...
*/
 ...
 // if ($user->ID!=1) { // can add this in so admin can have posts
    $restricted = array('Posts', 'Comments', 'Tools');
    end ($menu);
    while (prev($menu)){
